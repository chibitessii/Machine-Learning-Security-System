...
def CNN_plateRec(test_image_path):
	def load_model(path):
		try:
	...
	for c in sort_contours(cont):
		...
		ratio = h/w
		if ratio>=1: # Only select contour with defined ratio
			print("enters first if")
			if h/plate_image.shape[0]>=0.01: # Select contour which has the height larger than 50% of the plate
